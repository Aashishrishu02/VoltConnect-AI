...
import { Wallet as WalletIcon, ArrowUpRight, ArrowDownLeft, CreditCard, PlusCircle, History, X, QrCode, CheckCircle2, ShieldCheck, Smartphone } from 'lucide-react';
...
export const WalletPage: React.FC = () => {
  ...
  const handleProcessUpiPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    const depositNum = parseFloat(topUpAmount) || 500;

    if (upiMethod === 'VPA' && !vpaId.trim()) {
    ...
    // Simulate authentic Razorpay / UPI Bank Gateway Handshake (2 seconds)
    setTimeout(async () => {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 text-slate-900 dark:text-white">
      ...
      <div className="p-8 rounded-3xl bg-gradient-to-tr from-slate-900 via-slate-800 to-slate-900 border border-slate-700/80 text-white shadow-2xl relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-400 font-medium">Instant UPI Auto-Credit • Google Pay, PhonePe, Paytm & BHIM</p>
        </div>
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 shadow-sm space-y-4">
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700/50 text-xs">
          {[
            { id: 't1', description: 'UPI Top-Up via GPay (Ref: UPI/2026/994812)', amount: 500, type: 'DEPOSIT', createdAt: new Date().toISOString() },
      ...
      {/* Complete UPI Top-Up Modal Gateway */}
      {showTopUp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl relative text-slate-900 dark:text-white space-y-4">
            <button
              type="button"
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <div>
                <h3 className="font-extrabold text-lg text-slate-900 dark:text-white">Razorpay UPI Payment Gateway</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Instant Wallet Top-Up via Unified Payments Interface</p>
              </div>
            </div>

            {paymentStep === 'INPUT' && (
              <form onSubmit={handleProcessUpiPayment} className="space-y-4 text-xs">
                {/* Amount Presets */}
                <div>
                ...
                {/* UPI Mode Selector */}
                <div>
                  <label className="block text-xs font-semibold uppercase text-slate-400 mb-1.5">Select UPI Payment Method</label>
                ...
                {/* Option 3: Dynamic BHIM UPI QR Code */}
                {upiMethod === 'QR' && (
                  <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-center space-y-2">
                    <p className="font-bold text-slate-400">Scan with any UPI App (GPay, PhonePe, Paytm):</p>
                ...
                >
                  ...
                  Pay ₹{topUpAmount} via UPI & Top Up
                </button>
              </form>
            )}
            ...
            {paymentStep === 'PROCESSING' && (
              <div className="py-8 text-center space-y-4">
                ...
                <div>
                  <h4 className="font-extrabold text-base text-slate-900 dark:text-white">Connecting to UPI Bank Server...</h4>
                  <p className="text-xs text-slate-400">Please approve payment request of ₹{topUpAmount} on your UPI app</p>
                </div>
              </div>
            )}
            ...
            {paymentStep === 'SUCCESS' && (
              <div className="py-6 text-center space-y-4">
                ...
                <div>
                  <h4 className="font-extrabold text-lg text-emerald-500">UPI Payment Successful!</h4>
                  <p className="text-xs text-slate-400">₹{topUpAmount} has been credited to your ChargeMitra digital wallet.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setShowTopUp(false)}
